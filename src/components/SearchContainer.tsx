import { IonContent, IonItem, IonLabel, IonList, IonModal, IonButton, IonSearchbar } from '@ionic/react';
...
class SearchContainer extends React.Component<IContainerProps, IContainerState> {
  service: Service;
  ...
  async fetchData() {
    const { data } = await this.service.getBikes({name: this.state.searchName});
    ...
    return (
      <IonContent class="container">
          <IonSearchbar
            value={searchName}
            debounce={1000}
            onIonChange={(e) => this.setSearchName(e)}></IonSearchbar>
        <IonList>
          {this.generate(bikes)}
        </IonList>
      </IonContent>
